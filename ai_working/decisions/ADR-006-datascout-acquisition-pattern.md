...
| Priority | Source | Status | Transport | Value | Blocker |
|----------|--------|--------|-----------|-------|---------|
| **P0a** | **Epoch AI Capabilities Index (ECI)** | **Scouted ✅ — ready to build** | **HTTPS download** (`epoch.ai/data/eci_benchmarks.csv`, CC-BY, no registration) | Populates `gptval_benchmarks` table — 147 models × 39 benchmarks, composite ECI scores per model era; enables aggregate "rising waterline" narrative and `GET /gdpval/waterline` velocity endpoint | None — public, free, no API key |
| **P0b** | **GDPval task evaluations via Claude API** | **Scouted ✅ — ready to build** | **Compute** (`compute_gdpval_waterline.py` — NOT a download; runs 220 `gdpval_tasks` prompts through Claude API, grades via Haiku judge, stores in `gdpval_evaluations`) | Populates `gdpval_evaluations` table — task-level completion_pct per model era; enables occupation-level waterline velocity and "next to flip" signals | Anthropic API key (available); ~$40–55 for 3 Sonnet eras, ~$100–145 for all 4 including Opus; schema in place (migration 013) |
...
**Notes on P0a/P0b split:**
- "GPTVal" as referenced in `CLAUDE.md` and `fr8-role-evolution.md` is project-internal shorthand — no external dataset by that name exists.
- The concept maps to two real things at different granularities: P0a (aggregate model capability via Epoch ECI → `gptval_benchmarks`) and P0b (task-level completion via Claude API → `gdpval_evaluations`).
- P0a is a standard download acquisition (free, no cost). P0b is a **compute acquisition** — the only one in the backlog. Build P0a first (zero cost, unblocks the waterline endpoint); trigger P0b when API budget is approved.
- The `gptval_benchmarks` schema (`metric_name TEXT, model_era TEXT, measurement_date DATE, score FLOAT`) maps naturally to ECI's `(benchmark, model, date, score)` structure.
