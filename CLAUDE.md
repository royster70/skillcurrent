...
- **Derived-verdict snapshots are append-only** (ADR-012, `snapshot_runs` + `exposure_snapshots`) — each pipeline run appends a compact copy of the derived verdicts (occupation/sector/task/au β·zone·drift) via the terminal `snapshot_derived_products` stage; rows are never mutated or deleted, so "what changed over time" is a join between two `snapshot_runs`. The live derived tables stay clear-and-reload; zones are computed on the canonical thresholds, never re-derived
