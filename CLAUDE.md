...
Tier 1 (parallel track — no blockers):
  ...
  [x] FR-8.6/FR-8.7 GPTVal Waterline: migration 017 (gptval_benchmarks); scripts/ingest_epoch_eci.py (Epoch AI ECI, CC-BY runtime download → ≥408 rows; 464 as of 2026-07 — upstream CSV grows, treat runtime-download counts as floors); ERA_MAP 50+ model groups → platform era keys; GET /api/v1/gdpval/waterline (velocity per benchmark, sorted by descending rate of improvement); epoch_eci stage added to run_pipeline.py DAG
      FR-8.7 P0b: ⚠ gdpval_evaluations LOST in 2026-07 laptop rebuild (was 220 claude-4-sonnet @ avg 46.8% + 5 claude-4.5-sonnet; paid API output written only to DB, no dump existed). **Restore path decided: Sonnet subject is sufficient** — the judge is already Haiku 4.5 (`JUDGE_MODEL`), and the subject model is the *measured* variable, so run the two Sonnet eras (`compute_gdpval_waterline --eras claude-4-sonnet claude-4.5-sonnet`) for a full 2-point temporal waterline. **Real cost ~$15.70 (~$7.85/era: $4.72 target + $3.14 Haiku judge)** — the earlier "~$50 era 1 + ~$8 era 2" note overstated it. Opus eras (claude-4-opus/4.5-opus, ~$26.72 each, 3.4×) are OPTIONAL — they add the *tier-separation* narrative (Sonnet-vs-Opus capability band within a generation), not needed for velocity. Pending only an account recharge. Back up this table before any future rebuild (see REBUILD_RUNBOOK Phase 2).
